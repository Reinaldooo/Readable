  ...
  return (
    <li className="list-group-item list-group-item-action flex-column align-items-start comments">
      ...
      <div className="btn-group" role="group" aria-label="up and downvote">
        <button onClick={() => props.rateComment(UP, comment.id, indexComment, indexPost, comment.voteScore + 1)} type="button" className="button"><i className="fa fa-thumbs-up" aria-hidden="true"></i></button>
        <button onClick={() => props.rateComment(DN, comment.id, indexComment, indexPost, comment.voteScore - 1)} type="button" className="button"><i className="fa fa-thumbs-down" aria-hidden="true"></i></button>
      </div>
